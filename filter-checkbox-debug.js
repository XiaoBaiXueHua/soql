...
var fandomName = function () {
	...
	console.log(`fandomName fandom: ${fandom}`);
	console.log(`fandomName fandomCount: ${fandomCount}`);
	console.log(`fandomName tagCount: ${tagCount}`);
	console.log(`tag ownership percent: ${fandomCount / tagCount * 100}`);
...
var g_storage = localStorage["enable-global"];
var f_storage = localStorage[`enable-${cssFanName}`];

//vars to see if the checkmark's enabled
function g_enable() {return JSON.parse(g_storage)};
function f_enable() {
	if (!f_storage) {localStorage.setItem(`enable-${cssFanName}`, true)};
	return JSON.parse(localStorage[`enable-${cssFanName}`]);
}
...
function saveCheck(name, box, v) {
	...
	if (!checkk) {
		...
		box.addEventListener("click", function () {
			localStorage.setItem(`enable-${name}`, box.firstChild.checked);
			v = box.firstChild.checked;
			console.log(`${name} is enabled: ${v}`);
		})
	} else {
...
saveCheck("global", globCheck, g_enable());
...
if (!searchdt) {
	...
	if (fandomName) {
		...
		saveCheck(cssFanName, fanCheck, f_enable());
	}
...
var f_val = fandomName ? fandomBox.value : null;
...
/* form submit time */
function submission() {
	g_enable = document.querySelector("#enable-global").checked;
	console.log(`in the submission moments, g_enable is: ${g_enable()}`);
